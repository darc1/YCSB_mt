...
public class JdbcMtDBClientTest {
    ...
    private static JdbcMtDBClient jdbcDBClient = null;
    ...
    public static void setupWithBatch(int batchSize, boolean autoCommit) {
      try {
        ...
        jdbcDBClient = new JdbcMtDBClient();

        Properties p = new Properties();
        p.setProperty(JdbcMtDBClient.CONNECTION_URL, TEST_DB_URL);
        p.setProperty(JdbcMtDBClient.DRIVER_CLASS, TEST_DB_DRIVER);
        p.setProperty(JdbcMtDBClient.CONNECTION_USER, TEST_DB_USER);
        p.setProperty(JdbcMtDBClient.DB_BATCH_SIZE, Integer.toString(batchSize));
        p.setProperty(JdbcMtDBClient.JDBC_BATCH_UPDATES, "true");
        p.setProperty(JdbcMtDBClient.JDBC_AUTO_COMMIT, Boolean.toString(autoCommit));
        ...
        e.printStackTrace();
        fail("Could not create JdbcMtDBClient instance");
      }
    }
