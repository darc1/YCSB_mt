 ...
 * shorter may decrease the required storage size (default: "field")
 * </ul>
 */
public class CoreWorkload extends Workload {
   ...
   * "uniform", "zipfian" (favouring short records), "constant", and "histogram".
   * <p>
   * If "uniform", "zipfian" or "constant", the maximum field length will be that
   * specified by the fieldlength property. If "histogram", then the histogram
   * will be read from the filename specified in the "fieldlengthhistogram"
   * property.
   */
   ...
   * to match string sort order. Controls the number of 0s to left pad with.
   */
  ...
  public void init(Properties p) throws WorkloadException {
    ...
    if (requestdistrib.compareTo("uniform") == 0) {
      ...
      double frac = Double.parseDouble(
          p.getProperty(ExponentialGenerator.EXPONENTIAL_FRAC_PROPERTY, ExponentialGenerator.EXPONENTIAL_FRAC_DEFAULT));
    ...
    insertionRetryInterval = Integer
        .parseInt(p.getProperty(INSERTION_RETRY_INTERVAL, INSERTION_RETRY_INTERVAL_DEFAULT));
  }
   ...
   * client threads, this function must be thread safe. However, avoid
  ...
  public boolean doInsert(DB db, Object threadstate) {
    ...
    return doInsertInternal(db, dbkey, values);
  }

  protected boolean doInsertInternal(DB db, String dbkey, HashMap<String, ByteIterator> values) {
    Status status;
    int numOfRetries = 0;
    do {
   ...
   * label "VERIFY". Bucket 0 means the expected data was returned. Bucket 1 means
   * incorrect data was returned. Bucket 2 means null data was returned when some
   * data was expected.
   */
  ...
  protected static DiscreteGenerator createOperationGenerator(final Properties p) {
    ...
    final double readmodifywriteproportion = Double
        .parseDouble(p.getProperty(READMODIFYWRITE_PROPORTION_PROPERTY, READMODIFYWRITE_PROPORTION_PROPERTY_DEFAULT));
